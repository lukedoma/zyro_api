    ...
    try {
        ...
        if (existingReaction) {
            if (existingReaction.type === type) {
                ...
                await db.collection('reels').updateOne({ _id: new mongodb_2.ObjectId(reelId) }, { $inc: { likes: -1 } });
                ...
                await db.collection('reactions').updateOne({ _id: existingReaction._id }, { $set: { type, createdAt: new Date() } });
                return Response.json({
                    reacted: true,
                    type,
