  ...
  try {
    ...
    if (!reelId || !type) {
      return Response.json(
        { error: 'Reel ID and interaction type are required' },
        { status: 400 }
      );
    }
    ...
    const reel = await db.collection('reels')
      .findOne({ _id: new ObjectId(reelId) });
    ...
    const interaction = {
      ...
      type,
      weight: weights[type as keyof typeof weights],
      metadata: metadata || {},
      createdAt: new Date(),
    };
    ...
    const populatedReels = await Promise.all(
      reels.map(async (reel) => {
        const creator = await db.collection('users')
          .findOne(
            { _id: new ObjectId(reel.creatorId) },
            { projection: { displayName: 1, avatar: 1, username: 1, isVerified: 1 } }
          );
